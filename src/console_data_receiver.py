...
class ConsoleDataReceiver(object):
	...
	def __init__(self, PORT):
		...
		rospy.loginfo("Start console data receiver")
		...
		self.topic_name = "/console_cmd"

		## This prepublish is required one time before, otherwise the first publish wouldn't be sent on loop
		self.console_cmd_pub = rospy.Publisher(self.topic_name, ConsoleCmd, queue_size=10)
		self.console_cmd_pub.publish(self.console_cmd_msg)
		print("pre-published")
				...
				for i in dec.items():
					if i[0] == "id":
						...
						if dec["mission"] is not None:
							self.console_cmd_msg.mission.data = str(dec["mission"])
					elif i[0] == "cmd":
						for j in i[1].items():
							if j[0] == "mode":
								if dec["cmd"]["mode"] is not None:
									self.console_cmd_msg.mode.data = str(dec["cmd"]["mode"])
							elif j[0] == "dir":
								if dec["cmd"]["dir"] is not None:
									self.console_cmd_msg.dir.data = dec["cmd"]["dir"]
							elif j[0] == "servo":
								if dec["cmd"]["servo"] is not None:
									self.console_cmd_msg.servo.data = [dec["cmd"]["servo"]]
							elif j[0] == "inject":
								if dec["cmd"]["inject"] is not None:
									button_list[0] = dec["cmd"]["inject"]
							elif j[0] == "switch":
								if dec["cmd"]["switch"] is not None:
									button_list[1] = dec["cmd"]["switch"]


				self.console_cmd_msg.button.data = button_list
...
if __name__ == "__main__":
	...
	cdr = ConsoleDataReceiver(CONSOLE_PORT)
